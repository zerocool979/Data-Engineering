...
def cli_loop():
    ...
    while True:
        command = input(Fore.CYAN + "input somethin' here pleasee... > ").strip()
        ...
        if "error" in parsed:
            print(Fore.RED + "[ERROR]", parsed["error"])
            continue
        ...
        if parsed["output"] == "pdf":
            ...
            print(Fore.RED + "[INFO] No output file requested")

        elif parsed["output"] in ["csv","json","xlsx"]:
            ...
            print(Fore.RED + "[ERROR] Output format not implemented yet")
...
def list_files():
    ...
    if not files:
        print(Fore.RED + "Directory is empty\n")
        return
...
def read_file(command):
    ...
    if len(parts) < 2:
        print(Fore.REED + "[ERROR] Usage: cat <filename>\n")
        return
    ...
    try:
        ...
        print(Fore.RED + "[ERROR] Cannot read file\n")
...
def delete_file(command):
    ...
    if len(parts) < 2:
        print(Fore.RED + "[ERROR] Usage: rm <filename>\n")
        return
    ...
    print(Fore.RED + f"[INFO] File deleted: {filename}\n")
...
def export_data(comments, sentiment, filename, format):
    ...
    print(f"[INFO] File saved: {filename}.{format}")
...
def generate_pdf_report(filename, url, original_comments, top_words, sentiment_result):
    ...
    elements.append(Image("sentiment.png", width=5 * inch, height=3 * inch))
    elements.append(Spacer(1, 0.2 * inch))
...
def main(url, max_comments=MAX_COMMENTS):
    ...
    try:
        ...
        print("[ERROR] Failed to download comments")
        return [], [], (0,0,0)
    
    comment_list = []

    for c in tqdm(comments, desc="Scraping comments"):
    ...
    generate_sentiment_chart((positive, negative, neutral))
